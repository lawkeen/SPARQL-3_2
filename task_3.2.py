...
# Образовательные учреждения, в которых учился режиссер, на карте

movie_names = pd.read_csv("movie_names.csv", header=None, delimiter=', ', index_col=0, engine='python')

def institutions_query(movie_names):
    recommended_movie = list(rate_the_films_2(15, 4))[0].lstrip()
    movieName = movie_names[movie_names.index == recommended_movie][1][0]
    ...
    movie = res.json()['search'][0]['id']

    sparql = SPARQLWrapper("https://query.wikidata.org/sparql")
    sparql_query = """

    SELECT ?director ?directorLabel ?institutionLabel ?geoloc
    WHERE {
      ?film wdt:P31 wd:Q11424 filter(?film = wd:""" + movie + """).
      ?film wdt:P57 ?director.
      ?director p:P69 ?statement.
    ...
    resultsQuery = sparql.query().convert()

    results_df = pd.json_normalize(resultsQuery['results']['bindings'])
    if results_df.shape[0] == 0:
        print('Запрос не дал результатов')
    else:
        print(results_df[['director.value', 'directorLabel.value', 'institutionLabel.value', 'geoloc.value']].head(
            results_df.shape[0]))


institutions_query(movie_names)
